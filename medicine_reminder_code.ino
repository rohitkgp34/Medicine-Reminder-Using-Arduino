...
void setup() {
  ...
  Serial.begin(9600);  
  servo_test.attach(10);// start serial debugging
...
//    rtc.adjust(DateTime(F(__DATE__), F(__TIME__)));            // uncomment this to set the current time and then comment in next upload when u set the time
  rtc.adjust(DateTime(2019, 1, 10, 7, 59, 50));                // manual time set
    ...
    if (int(now.minute()) >= buzz8amMM) {
      if (int(now.second()) > buzz8amSS) {
        ...
        startBuzz();
            lcd.clear();
            lcd.setCursor(0, 0);
            lcd.print("Take Pill 1"); 
          for(angle = 0; angle < 90; angle += 1)    // command to move from 0 degrees to 180 degrees 
  {                                  
    servo_test.write(angle); 
    timeScreen();          //command to rotate the servo to the specified angle
    delay(15);                       
  } 
